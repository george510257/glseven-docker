...
| phpLDAPadmin | http://php-ldap-admin.glseven.local:8080 | LDAP 管理（USER ID：`admin`；完整 DN `cn=admin,dc=glseven,dc=com` 等效） |
...
- **phpLDAPadmin 登录**：登录者必须是目录内真实条目，而 osixia/openldap 只把 `cn=admin` 设为 olcRootDN（DB 级 bind 凭据，无目录条目），因此仓库附 `openldap/bootstrap/admin-entry.ldif`（含 `uid=admin`），首次初始化卷后需导入一次（命令见该文件头注释），导入后 USER ID 填 `admin` 登录（bind 实际走 olcRootDN，具全权）。登录后若见一次 500 页，刷新即可（登录表单闪存残留，一次性）。另：上游镜像 `config/ldap.php` 遗漏 `base_dn` 键，已由挂载的修正版 `phpldapadmin/conf/ldap.php` 补齐。
