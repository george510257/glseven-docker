...
6. 每个镜像一个二级域名（=容器名，零别名）；多端点实例同域名不同端口——HTTP 端点走 :8000 虚拟主机，TCP 端点走 stream 原生端口。
...
**已否决的备选**：路径前缀方式（open-webui 不支持子路径，各应用需改 base-path）；统一号段重排（违背“默认端口”原则）；UI 保留直连后备端口（与端口收敛目标冲突）；同实例多端点独立子域名（es、apisix-admin）——违背「每个实例一个二级域名」。
...
│ :8000 http 块     → 18 个 server 块（根域 + 17 个实例 vhost）      │
...
### 4.2 http 子域名路由（:8000，18 个 server 块）
...
| `elk.glseven.local` | elk:5601 | Kibana UI（ES API 走同域名 :9200 stream） |
...
| `apisix.glseven.local` | apisix:9080 | 数据面（Admin API 走同域名 :9180 stream） |
| `ollama.glseven.local` | ollama:11434 | |
| `open-webui.glseven.local` | open-webui:8080 | |
| `moontv.glseven.local` | moontv:3000 | |

**命名规则**：二级域名 = 容器名（如 `nexus3`、`php-ldap-admin`、`xxl-job-admin`），**零别名**（根域 `glseven.local` 为静态门户，不属服务域名）。多端点实例**同域名不同端口**：HTTP 端点走 :8000 虚拟主机（如 `elk.glseven.local:8000` → Kibana），TCP 端点走 stream 原生端口（如 `elk.glseven.local:9200` → ES API、`apisix.glseven.local:9180` → Admin API）。无 HTTP 端点的服务（mysql/redis/mongo/kafka/openldap）其二级域名仅作 DNS 解析（/etc/hosts → 127.0.0.1），配合 stream 原生端口使用，无需 nginx 配置。
...
| portainer | 子域名 | — |
| apisix | 子域名(9080) + stream 9080/9180 | 9091 (Prometheus 抓取) |
| ollama | stream 11434 | — |
...
- `portal/conf/default.conf`（**重写**）：18 个 server 块（根域静态 + 17 个实例 vhost）
...
| `portal/conf/default.conf` | 重写 | 18 个 server 块：根域静态 + 17 个实例 vhost（§4.2/§5.2） |
| `docker-compose-portal.yml` | 修改 | 挂载 nginx.conf；ports 改为 8000 + 18 个 stream 端口；新增 healthcheck |
...
4. 浏览器验证：18 个子域名站点逐个访问，登录页/首页正常（需先完成 /etc/hosts 初始化）。
...
9. 子域名 API 验证：`curl http://etcd.glseven.local:8000/version`、`curl http://ollama.glseven.local:8000/api/tags`、带 X-API-KEY 请求 `http://apisix.glseven.local:9180/apisix/admin/routes`（同域名不同端口，走 stream）。
