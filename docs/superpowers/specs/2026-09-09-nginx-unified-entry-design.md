...
│ :8000 http 块     → 15 个子域名 vhost（Web UI / 静态门户根）       │
│ :3306…:9180 stream → 17 条 TCP 透明转发（原生端口 → 容器原生端口）  │
└──────────────────────────────┬───────────────────────────────────┘
                               ▼ glseven 网络内按服务名转发
...
- `portal/conf/nginx.conf`（**新增**，挂载覆盖 `/etc/nginx/nginx.conf`）：
  ...
  - `stream` 块：17 条 `upstream` + `server` 转发
- `portal/conf/default.conf`（**重写**）：15 个子域名 server 块 + 根域静态站点
...
macOS 解析器不支持 hosts 通配符，且浏览器仅对 `*.localhost` 免配置；`*.glseven.local` 需精确条目。15 个主机名压缩为**一行追加**（hosts 支持一行多别名），README 提供复制即用命令（需 sudo）：

```
127.0.0.1 glseven.local grafana.glseven.local prometheus.glseven.local kibana.glseven.local mongo-express.glseven.local adminer.glseven.local rabbitmq.glseven.local phpldapadmin.glseven.local keycloak.glseven.local nacos.glseven.local xxl-job.glseven.local nexus.glseven.local portainer.glseven.local open-webui.glseven.local moontv.glseven.local
```
...
| `portal/html/index.html` | 修改 | 卡片链接改为子域名 URL，分组结构保留 |
...
3. TCP 连通矩阵：对 17 个 stream 端口逐项 `nc -vz 127.0.0.1 <port>`；mysql/redis 用真实客户端登录验证。
4. 浏览器验证：15 个子域名逐个访问，登录页/首页正常（需先完成 /etc/hosts 初始化）。
5. WebSocket：portainer 控制台可进入、grafana 面板 live 刷新。
6. SSE 流式：open-webui 对话流式输出平滑不卡顿。
7. Registry：`docker push localhost:5000/<image>`（localhost 端口自动视为 insecure）。
8. Keycloak：经 `keycloak.glseven.local` 登录 admin 控制台，回调 URL 正确（验证 KC_PROXY_HEADERS）。
9. Kafka/Nacos 透明性：本地客户端按原生端口连接 kafka 9092、nacos 8848/9848 成功。
...
5. **registry 5000 是默认端口原则下唯一需宿主侧配合的端口**（其余 17 条 stream 端口均无冲突风险）。
